...
import { getFirestore, collection, query, where, getDocs, doc, setDoc } from "https://www.gstatic.com/firebasejs/11.6.0/firebase-firestore.js";
...
async function loadProducts() {
  try {
    ...
    if (filterValue === 'reviewed') {
      filteredProducts = products.filter(p => reviewedProducts.includes(p.productId));
...
async function displayProducts(products) {
  ...
  products.forEach(product => {
    ...
    const isReviewed = reviewedProducts.includes(product.productId);
...
async function submitReview() {
  ...
  try {
    // Use productId for uniqueness and link
    const reviewRef = doc(db, "reviews", `${currentUser.uid}_${currentProduct.productId}`);

    await setDoc(reviewRef, {
      userId: currentUser.uid,
      productId: currentProduct.productId, // <-- Store productId!
...
async function getReviewedProducts() {
  ...
  const querySnapshot = await getDocs(q);
  return querySnapshot.docs.map(doc => doc.data().productId); // Use productId!
}
