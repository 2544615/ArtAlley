describe('Seller Signup Page', () => {
  beforeEach(() => {
    ...
    cy.visit('http://localhost:3000/SignUp Folder/seller-signup.html'); // Adjust path if needed
  });

  it('Loads the signup form correctly', () => {
    cy.contains('Welcome to Art Alley').should('be.visible');
    cy.contains('Sign Up as a seller').should('be.visible');
  });
  ...
  it('Prevents form submission without checking Terms and Conditions', () => {
    cy.get('#username').type('seller123');
    cy.get('#address').type('seller@example.com');
    ...
    // Check that the form still exists (submission didn't go through)
